...
public:
    ...
    bool isWrongLeader;
    bool isKeyExist;
    friend Serializer& operator >> (Serializer& in, DelReply& d) {
		in >> d.value >> d.isWrongLeader >> d.isKeyExist;
		return in;
	}
	friend Serializer& operator << (Serializer& out, DelReply d) {
		out << d.value << d.isWrongLeader << d.isKeyExist;
		return out;
	}
};
...
public:
    ...
    string delete_(string key);                                    //定义的对于kvServer的get请求
...
string Clerk::delete_(string key){
    GetArgs args;
    args.key = key;
    args.clientId = clientId;
    args.requestId = getCurRequestId();
    ...
    while(1){
        buttonrpc client;
        client.as_client("127.0.0.1", servers[cur_leader][curPort]);
        DelReply reply = client.call<DelReply>("delete_", args).val();      //取得RPCreply，对于get需要有返回值value
        if(reply.isWrongLeader){
            cur_leader = getChangeLeader();
            usleep(1000);
        }else{
